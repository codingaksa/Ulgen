...
const isOwner = (server, uid) =>
  server?.owner && (server.owner.equals?.(uid) || String(server.owner) === String(uid));
...
const createInviteController = async (req, res) => {
  try {
    ...
    const { serverId, channelId, inviteType, remainingUses, expires } = req.body || {};
    ...
    if (channelId) {
      const ch = await Channel.findOne({ _id: channelId, server: server._id }).select('_id');
...
const getInviteInfo = async (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, message: 'Failed to get invite info' });
  }
};
...
const redeemInvite = async (req, res) => {
  try {
    ...
    const invite = await Invite.redeemByTokenRaw(token, { returnPublic: false });
    if (!invite) {
    ...
    const alreadyServer = server.members?.some((m) => String(m.user) === String(requesterId));
    if (!alreadyServer) {
    ...
    if (invite.inviteType === 'channel' && invite.channelId) {
      const channel = await Channel.findOne({ _id: invite.channelId, server: invite.serverId });
